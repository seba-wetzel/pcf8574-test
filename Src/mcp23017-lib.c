...
// Lee un registro dado
Status mcp23017_read(MCP23017_HandleTypeDef *hdev, uint16_t reg, uint8_t *data) {
	return HAL_I2C_Mem_Read(hdev->hi2c, hdev->addr, reg, 1, data, 1,
	I2C_TIMEOUT);
}

// Escribe en un registro un valor
Status mcp23017_write(MCP23017_HandleTypeDef *hdev, uint16_t reg, uint8_t *data) {
	return HAL_I2C_Mem_Write(hdev->hi2c, hdev->addr, reg, 1, data, 1,
	I2C_TIMEOUT);
}

// Configura un pin como entrada o salida de un puerto
Status mcp23017_iodir(MCP23017_HandleTypeDef *hdev, uint8_t port, uint8_t iodir) {
	uint8_t data[1] = { iodir };
	return mcp23017_write(hdev, REGISTER_IODIRA | port, data);
}

// Configura la polaridad de un pin en un puerto
Status mcp23017_ipol(MCP23017_HandleTypeDef *hdev, uint8_t port, uint8_t ipol) {
	uint8_t data[1] = { ipol };
	return mcp23017_write(hdev, REGISTER_IPOLA | port, data);
}

// Configura las resistencias pullUp
Status mcp23017_ggpu(MCP23017_HandleTypeDef *hdev, uint8_t port, uint8_t pu) {
	uint8_t data[1] = { pu };
	return mcp23017_write(hdev, REGISTER_GPPUA | port, data);
}

// Lee el estado de un pin de un puerto
Status mcp23017_read_gpio(MCP23017_HandleTypeDef *hdev, uint8_t port) {
	uint8_t data[1];
	Status status;
...
// Escribe un estado en un pin de un puerto
Status mcp23017_write_gpio(MCP23017_HandleTypeDef *hdev, uint8_t port) {
